...
// Cliente Admin para criar usuários (precisa da Service Role Key)
// Nota: Em produção, garanta que SUPABASE_SERVICE_ROLE_KEY está nas env vars da Vercel
...
export async function criarUsuario(data: {
  ...
  try {
    ...
    await prisma.usuario.create({
      data: {
        id: undefined, // Prisma gera autoincrement se configurado, ou uuid
        nome: data.nome,
        email: data.email,
        tipo: data.tipo,
        senha: '', // Não salvamos a senha aqui por segurança, o Auth gerencia
        ativo: true,
...
export async function excluirUsuario(id: number, email: string) {
  try {
    // 1. Buscar ID do Supabase (poderíamos salvar no banco para facilitar)
